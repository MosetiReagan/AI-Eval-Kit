...
describe("Providers System", () => {
  ...
  it("MockProvider simulates latency, token usage, and errors", async () => {
    ...
    await expect(
      mock.chat([{ role: "user", content: "fail-now" }]),
    ).rejects.toThrow("Rate limit simulation");
  });
  ...
  it("ProviderRegistry manages and creates providers", () => {
    ...
    expect(defaultProviderRegistry.get("custom-mock")?.name).toBe(
      "custom-mock",
    );
  });
  it("throws InvalidOutputError when messages array is empty", async () => {
    const mock = new MockProvider();
    await expect(mock.chat([])).rejects.toThrow(
