...
describe("Dataset System", () => {
  it("loads YAML datasets correctly", async () => {
    const datasetPath = path.resolve(
      __dirname,
      "../fixtures/simple-llm/dataset.yaml",
    );
  ...
  it("samples dataset with limit and seed", () => {
    ...
    const sampled = filterDataset(mockDataset, {
      limit: 3,
      random: true,
      seed: 1234,
    });
  ...
  it("validates dataset and produces analysis stats", () => {
    ...
    expect(stats.warnings.some((w) => w.includes("Duplicate case id"))).toBe(
      true,
    );
    expect(
      stats.warnings.some((w) => w.includes("does not specify expected")),
    ).toBe(true);
  });
});
