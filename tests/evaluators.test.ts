...
describe('Built-in Evaluators', () => {
  ...
  it('json_validity extracts and parses JSON correctly', async () => {
    const markdownJson = 'Here is your data:\n```json\n{"status": "ok", "code": 200}\n```\nHope that helps!';
  ...
  it('hallucination evaluator flags ungrounded statements', async () => {
    const context = 'Apollo 11 landed on the Moon on July 20, 1969. Neil Armstrong was commander.';
    const grounded = await hallucinationEvaluator.evaluate({
      input: 'test',
      context,
      actual: { output: 'Neil Armstrong was the commander during the Apollo 11 Moon landing in July 1969.' },
    ...
    const hallucinated = await hallucinationEvaluator.evaluate({
      input: 'test',
      context,
      actual: { output: 'Apollo 11 landed on Mars with a crew of five astronauts including Elon Musk.' },
      options: { threshold: 0.7 },
    });
  ...
  it('criteria and llm_judge evaluator heuristic fallback work offline', async () => {
    ...
    expect(judgeRes.score).toBeGreaterThan(0);
  });
});
