...
export class JunitReporter implements Reporter {
  ...
  format(
    ...
    for (const c of run.cases) {
      ...
      if (!c.passed) {
        const failedEvaluators = Object.entries(c.evaluatorResults).filter(
          ([_, r]) => !r.passed,
        );
        const reason = failedEvaluators
          .map(([name, r]) => `${name}: ${r.reason ?? "Failed"}`)
          .join("; ");
    ...
    if (regression?.hasRegression) {
      lines.push(
        '    <testcase classname="regression" name="baseline-check" time="0.001">',
      );
      lines.push(
