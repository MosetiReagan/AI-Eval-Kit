...
export class MarkdownReporter implements Reporter {
  ...
  format(
    ...
    lines.push("");
    lines.push(
      `**Status:** ${badge} | **Project:** ${run.projectName} | **Target:** ${run.targetName}`,
    );
    ...
    lines.push("| :--- | :--- |");
    lines.push(
      `| **Overall Score** | **${(run.overallScore * 100).toFixed(1)}%** |`,
    );
    lines.push(
      `| **Test Cases** | ${run.totalCases} total (${run.passedCases} passed, ${run.failedCases} failed) |`,
    );
    lines.push(
      `| **Pass Rate** | ${((run.passedCases / (run.totalCases || 1)) * 100).toFixed(1)}% |`,
    );
    lines.push(
      `| **Latency (Avg / P95)** | ${run.latencyStats.avgMs}ms / ${run.latencyStats.p95Ms}ms |`,
    );
    lines.push(
      `| **Total Tokens** | ${run.totalTokens.totalTokens.toLocaleString()} |`,
    );
    ...
    if (failedCases.length > 0) {
      lines.push("<details>");
      lines.push(
        `<summary><b>View ${failedCases.length} Failed Cases</b></summary>`,
      );
      lines.push("");
      ...
      for (const c of failedCases) {
        ...
        if (c.case.expected) {
          lines.push(
            `- **Expected:** \`${escapeHtml(JSON.stringify(c.case.expected))}\``,
          );
        }
        lines.push(
          `- **Actual:** \`${escapeHtml(c.output?.output ?? "none")}\``,
        );

        for (const [evName, res] of Object.entries(c.evaluatorResults)) {
          if (!res.passed) {
            lines.push(
              `- ❌ **${evName}**: ${(res.score * 100).toFixed(1)}% — ${escapeHtml(res.reason ?? "Failed")}`,
            );
          }
        }
        lines.push("");
    ...
    lines.push("---");
    lines.push(
      "*Generated automatically by [AI Eval Kit](https://github.com/reagan/ai-eval-kit)*",
    );
