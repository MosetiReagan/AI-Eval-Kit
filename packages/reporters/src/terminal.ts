...
export class TerminalReporter implements Reporter {
  ...
  format(
    ...
    lines.push(
      `${pc.bold("Target:")}      ${run.targetName}${run.modelName ? ` (${run.modelName})` : ""}`,
    );
    lines.push(pc.dim("----------------------------------------------------"));
    lines.push("");
    ...
    lines.push("");
    lines.push(
      `  ${pc.bold("Overall Score".padEnd(28))} ${pc.bold(overallColored)}`,
    );
    lines.push("");
    ...
    if (run.failedCases > 0 && !options.quiet) {
      ...
      for (let i = 0; i < displayCount; i++) {
        ...
        if (c.case.expected) {
          lines.push(
            `  ${pc.dim("Expected:")} ${JSON.stringify(c.case.expected)}`,
          );
        }

        const actualStr = c.output?.output
          ? c.output.output.replace(/\n/g, " ")
          : "No output";
        const truncatedActual =
          actualStr.length > 120 ? actualStr.slice(0, 117) + "..." : actualStr;
      ...
      if (failedCases.length > displayCount) {
        lines.push(
          pc.dim(
            `  ... and ${failedCases.length - displayCount} more failed cases (use --verbose to see all)`,
          ),
        );
        lines.push("");
      }
