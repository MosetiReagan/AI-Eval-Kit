...
export function parseCsvDataset(content: string, datasetName: string): Dataset {
  ...
  if (lines.length < 2) {
    throw new InvalidDatasetError(
      "CSV dataset must contain a header row and at least one data row",
    );
  }
  ...
  const inputIdx =
    headers.indexOf("input") !== -1
      ? headers.indexOf("input")
      : headers.indexOf("prompt");
  ...
  if (inputIdx === -1) {
    throw new InvalidDatasetError(
      'CSV dataset must contain an "input" or "prompt" column header',
    );
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const id =
      idIdx !== -1 && values[idIdx]
        ? values[idIdx]!
        : `case-${String(i).padStart(3, "0")}`;
    const inputVal = values[inputIdx] ?? "";
    const expectedVal =
      expectedIdx !== -1 && values[expectedIdx]
        ? values[expectedIdx]
        : undefined;
    const contextVal =
      contextIdx !== -1 && values[contextIdx] ? values[contextIdx] : undefined;
    const tagsVal =
      tagsIdx !== -1 && values[tagsIdx]
    ...
    cases.push({
      ...
      expected: expectedVal
        ? { exact: expectedVal, contains: expectedVal }
        : undefined,
      context: contextVal,
      tags: tagsVal,
    });
...
export function parseJsonlDataset(
  ...
  for (let i = 0; i < lines.length; i++) {
    try {
      ...
      throw new InvalidDatasetError(
        `Failed to parse JSONL line ${i + 1}: ${msg}`,
      );
    }
  }
...
export async function loadDataset(
  ...
  try {
    ...
    if (path.isAbsolute(filePath) && fs.existsSync(filePath)) {
      ...
      throw new InvalidDatasetError(
        `Path traversal detected: "${filePath}" resolves outside project directory.`,
      );
    }
  }
  ...
  if ([".ts", ".js", ".mjs"].includes(ext)) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      throw new InvalidDatasetError(
        `Failed to load dataset from JS/TS file: ${msg}`,
      );
    }
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    throw new InvalidDatasetError(
      `Failed to parse dataset syntax in ${path.basename(resolvedPath)}: ${msg}`,
    );
  }
  ...
  if (!validation.success) {
    const errors = validation.error.errors
      ...
      .join("\n");
    throw new InvalidDatasetError(
      `Dataset validation failed for ${path.basename(resolvedPath)}:\n${errors}`,
    );
  }
