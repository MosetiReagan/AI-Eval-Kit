...
export class GeminiProvider implements Provider {
  ...
  async chat(messages: ChatMessage[], options?: ProviderCallOptions): Promise<ProviderResponse> {
    ...
    try {
      ...
      throw new ProviderError(`Gemini network error: ${redactSecrets(msg)}`, undefined, true);
    }

    if (!response.ok) {
      const text = await response.text();
      if (response.status === 400 && text.includes('API_KEY_INVALID')) {
        throw new AuthenticationError(`Gemini API key is invalid: ${redactSecrets(text)}`);
      }
      if (response.status === 429) {
        throw new RateLimitError(`Gemini rate limited: ${redactSecrets(text)}`);
      }
      throw new ProviderError(`Gemini error (${response.status}): ${redactSecrets(text)}`, response.status);
    }
