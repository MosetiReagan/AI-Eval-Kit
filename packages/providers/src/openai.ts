...
export class OpenAICompatibleProvider implements Provider {
  ...
  async chat(
    ...
    try {
      ...
      throw new ProviderError(
        `Network request to ${url} failed: ${redactSecrets(msg)}`,
        undefined,
        true,
      );
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        throw new AuthenticationError(
          `Authentication failed with provider ${this.name}: ${redactSecrets(errorText)}`,
        );
      }
      if (response.status === 429) {
        throw new RateLimitError(
          `Rate limit exceeded for provider ${this.name}: ${redactSecrets(errorText)}`,
        );
      }
      throw new ProviderError(
  ...
  async embed(texts: string[]): Promise<number[][]> {
    ...
    if (!res.ok) {
      ...
      throw new ProviderError(
        `Embeddings failed: ${redactSecrets(err)}`,
        res.status,
      );
    }
