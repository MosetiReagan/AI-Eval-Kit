...
export function extractJsonFromText(text: string): { success: boolean; data?: unknown; error?: string } {
...
export const jsonValidityEvaluator = defineEvaluator({
  name: 'json_validity',
  description: 'Checks whether the output is valid JSON or contains a parseable JSON block',
...
export const jsonSchemaEvaluator = defineEvaluator({
  name: 'json_schema',
  description: 'Validates JSON output against required fields or a schema definition',
  evaluate: (ctx: EvaluatorContext): EvaluationResult => {
    const extracted = extractJsonFromText(ctx.actual.output);
    ...
    const schema = (ctx.options?.schema as Record<string, unknown>) ?? ctx.expected?.jsonSchema;
    if (!schema) {
      return {
        score: 1,
        passed: true,
    ...
    for (const [propName, propDef] of Object.entries(properties)) {
      if (propName in data && propDef.type) {
        ...
        if (actualType !== propDef.type) {
          typeMismatches.push(`${propName}: expected ${propDef.type}, got ${actualType}`);
        }
      }
    }

    const errors = [...missingFields.map((f) => `Missing required field "${f}"`), ...typeMismatches];
    ...
    return {
      ...
      passed,
      reason: passed ? 'JSON schema validation passed' : `Schema errors: ${errors.join(', ')}`,
