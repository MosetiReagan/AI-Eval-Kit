...
export const llmJudgeEvaluator = defineEvaluator({
  name: 'llm_judge',
  description: 'Uses an LLM as an impartial judge to score responses against criteria',
  evaluate: async (ctx: EvaluatorContext): Promise<EvaluationResult> => {
    ...
    const threshold = (ctx.options?.threshold as number) ?? (scale === '1-5' ? 3.5 : 0.7);
    ...
    const expectedStr = ctx.expected ? JSON.stringify(ctx.expected) : 'Not specified';
...
export const criteriaEvaluator = defineEvaluator({
  name: 'criteria',
  description: 'Evaluates output against custom natural-language criteria rules',
  evaluate: async (ctx: EvaluatorContext): Promise<EvaluationResult> => {
    const criteria =
      (ctx.options?.criteria as string[]) ??
      ctx.expected?.criteria ??
      [];
    ...
    for (const crit of criteria) {
      ...
      if (critLower.includes('not invent') || critLower.includes('not hallucinate')) {
      ...
      } else if (critLower.includes('must contain') || critLower.includes('must include')) {
        const keyword = crit.split(/must (?:contain|include)/i)[1]?.trim().replace(/['"]/g, '');
    ...
    const score = criteria.length > 0 ? passedCriteria.length / criteria.length : 1;
