...
export const ragContextRelevanceEvaluator = defineEvaluator({
  ...
  evaluate: (ctx: EvaluatorContext): EvaluationResult => {
    const query = typeof ctx.input === 'string' ? ctx.input : ctx.input.message ?? '';
    const context = typeof ctx.context === 'string' ? ctx.context : JSON.stringify(ctx.context ?? '');
    ...
    if (queryKeywords.length === 0) {
      return { score: 1, passed: true, reason: 'Empty query, skipping relevance check' };
    }
    ...
    return {
      ...
      metadata: { matchedKeywords: matched, totalQueryKeywords: queryKeywords.length },
    };
  },
});
...
export const ragContextRecallEvaluator = defineEvaluator({
  ...
  description: 'Measures whether the retrieved context contains the information needed to answer the question',
  evaluate: (ctx: EvaluatorContext): EvaluationResult => {
    const expected = ctx.expected?.exact ?? (typeof ctx.expected === 'string' ? ctx.expected : '');
    const context = typeof ctx.context === 'string' ? ctx.context : JSON.stringify(ctx.context ?? '');
    ...
    const score = expectedKeywords.length > 0 ? matched.length / expectedKeywords.length : 1;
    ...
    return {
      ...
      metadata: { matchedKeywords: matched, expectedKeywordsCount: expectedKeywords.length },
    };
  },
});
...
export const ragAnswerRelevanceEvaluator = defineEvaluator({
  ...
  description: 'Measures how directly the generated answer addresses the question',
  evaluate: (ctx: EvaluatorContext): EvaluationResult => {
    const query = typeof ctx.input === 'string' ? ctx.input : ctx.input.message ?? '';
    ...
    const score = queryKeywords.length > 0 ? matched.length / queryKeywords.length : 1;
...
export const ragCitationEvaluator = defineEvaluator({
  ...
  evaluate: (ctx: EvaluatorContext): EvaluationResult => {
    const answer = ctx.actual.output;
    const citationPattern = /\[(?:Source|Doc|Ref|\d+)[^\]]*\]|\((?:Source|Doc|\d+)[^)]*\)/gi;
